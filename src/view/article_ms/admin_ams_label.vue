      ...
      <el-col :span="16">
        <el-card class="box-card" style="width: 100%;">
          ...
          <el-table :data="tableData" stripe style="width: 100%">
            <el-table-column prop="labelId" label="ID"/>
      ...
      <el-col :span="8">
        <el-card shadow="hover">
          ...
          <el-form label-width="120px" :label-position="left">
            <el-form-item :model="form" label="标签名称">
...
const GetData = (size: number, current: number) => {
  api.getListAPI('/api/admin/ams/label', {'pageSize': size, 'currentPage': current}).then(res => {
    //数据处理
...
const SubmitAdd = () => {
  handleCofirm().then(res => {
    ...
    api.getListAPI('/api/admin/ams/labeladd', form).then(res => {
      //数据处理
      const code = res.data.code
...
const handleDelete = (tem: any, temp: any) => {
  handleCofirm().then(res => {
    ...
    api.getListAPI('/api/admin/ams/labeldelete?id=' + temp.labelId).then(res => {
      //数据处理
      const code = res.data.code
      let data = res.data.data.item
      if (code === 200) {
